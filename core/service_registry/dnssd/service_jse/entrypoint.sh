...
override_property_if_given   "ping.timeout"        "$PING_TIMEOUT"
override_property_if_given   "ping.scheduled"      "$PING_SCHEDULED"
override_property_if_given   "ping.interval"       "$PING_INTERVAL"

# Use environment variables to override values in the dns.properties file.
PROPERTIES_FILE='config/app.properties'
ensure_trailing_newline
override_property_if_given   "tsig.name"           "$TSIG_NAME"
override_property_if_given   "tsig.key"            "$TSIG_KEY"
override_property_if_given   "tsig.algorithm"      "$TSIG_ALGORITHM"
override_property_if_given   "dns.ip"              "$DNS_IP"
override_property_if_given   "dns.port"            "$DNS_PORT"
...
if [ -z "$1" ]; then
    java -jar serviceregistry-M2.jar -d -m both &
else
    exec "$@" &
fi
