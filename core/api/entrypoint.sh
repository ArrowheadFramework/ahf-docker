...
if [ -n "${DB_HOST}" ]; then
    DB_ADDRESS="${DB_ADDRESS:-jdbc:mysql://${DB_HOST}:3306/arrowhead?autoReconnect=true&useUnicode=yes}"
    LOG4J_APPENDER_DB_URL="${LOG4J_APPENDER_DB_URL:-jdbc:mysql://${DB_HOST}:3306/log?autoReconnect=true&useUnicode=yes}"
fi
