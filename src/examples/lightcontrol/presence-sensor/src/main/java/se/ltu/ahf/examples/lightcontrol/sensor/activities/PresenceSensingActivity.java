...
public class PresenceSensingActivity implements Runnable {
    ...
    private final Client client = ClientBuilder.newBuilder()
            ...
            .sslContext(Configuration.getInstance().getSslContext()).build();
    ...
    public void run() {
        ...
        /* Perform any scheduled processing */
            ...
            WebTarget pushPresenceTarget = client.target(Configuration.getInstance().getLightControlEndpoint());
            ...
            System.err.println(Configuration.getInstance().getLightControlEndpoint());
        } finally {
            if (response != null) {
                response.close();
            }
